...
report 50100 "Block Non Satisfactory Items"
{
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            column(Description; Item.Description) { IncludeCaption = true; }
            column(NonSatisfactoryPurchQty; Item."Non-satisfactory Purch. (Qty.)") { IncludeCaption = true; }
...
#pragma warning disable AA0205
                Message('OnPreDataItem Item %1', this.Counter);
#pragma warning restore
            ...
            begin
#pragma warning disable AA0205
                Message('OnPostDataItem Item Total %1 Modificados %2', this.Counter, this.ModifiedCounter);
#pragma warning restore
            end;
        }
    }
    ...
    trigger OnPreReport()
    begin
        if this.NoOfNonSatisfactoryUnits = 0 then
            Error('Tienes que especificar un número mínimo');

        if not Confirm('¿Estás seguro que quieres bloquear los productos con más de %1 unidades no satisfactorias?', false, this.NoOfNonSatisfactoryUnits) then
